...
export const SaveDeliverableInputSchema = z.object({
  ...
  content: z.string().min(1).optional().describe('File content (markdown for plans/exec results). Optional if file_path is provided.'),
...
export function createSaveDeliverableTool(targetDir: string) {
  return tool(
    'save_deliverable',
    'Saves UAT deliverable files (plans, execution results, reports). For large reports, write the file to disk first then pass file_path instead of inline content to avoid output token limits.',
